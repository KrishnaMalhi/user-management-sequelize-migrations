...
const assignPermissionsToRole = async (req, res) => {
  ...
  try {
    ...
    await Promise.all(permissions.map(async (perm) => {
      ...
      const isPermissionExist = await PermissionsDBQuery.getPermissionById(permission);
    ...
    return ResponseUtils.sendResponse(res, req, {}, "Permissions assigned to role successfully", true, 200);
  } catch (err) {
